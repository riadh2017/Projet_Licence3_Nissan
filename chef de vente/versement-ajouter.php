...
<div id="container">
 ...
 <p id="title" >Nissan Djurdjura motors</p>	
	...
	<div id="left1">
			...
			<form action="versement-ajouter.php" method="POST">
               ...
               )
{
   ...
   $req1=$bdd->query('SELECT  command.id_client, command.id_command, client.nom, command.id_user, command.mode_de_vente, command.date_command ,client.tele,command.numero_bon,command.rest_a_payer FROM command, client WHERE (command.id_client=client.id_client)');
				...
				echo"<form action='ajouter_versement_post.php' method='POST'>";
			...
			echo '<tr class="resultat">
					...
					<input type="hidden" name="rest_a_payer"  value="'.$donnee['rest_a_payer'].'"/>
...
$i=0;
			...
			echo '<tr class="resultat">
				    ...
				    <td>'.$donnee3['numero_bon'].'</td>
				    <input  type="hidden" name="numero" value="'.$donnee3['numero_bon'].'" id="nomero1"/>
					...
					<input type="hidden" name="rest_a_payer"  value="'.$donnee3['rest_a_payer'].'"/>
    ...
    if(isset($_GET['riad']))
{
  ...
  if($_GET['riad']==1){
  echo"<script>alert('la somme verser  depasse le rest a payer du bon ');</script>";
}
}
