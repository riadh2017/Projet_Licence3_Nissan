			...
			<div id="section_saisie-modele">
				 <?php
			  			...
			  			$bdd=new pdo('mysql:host=localhost;dbname=nissan','root','');
			  		          ...
			  		          $rep = $bdd->query('SELECT * FROM modele_veh where  modele_veh.id_modele_veh=\'' . $_POST['id'] . '\' ');
			  		          $donn1 = $rep->fetch();
                 ?>
				<form method="post" action="vehicule-update-post.php">
			   <div id="modele">
                                                     <h1>modele de vehicule</h1>
						   <label class="text-att">designation</label> <input type="text" name="modele"  value="<?php echo $donn1['designation']; ?>" class="des"  required/>   
							<label class="text-att">montant  de la remise</label><input value="<?php echo $donn1['REMISE']; ?>" type="text" name="remise" class="des"  required/>
							<label class="text-att">HT HDD</label><input type="text"   value="<?php echo $donn1['HT_HDD']; ?>"name="hdd" class="des"  required/>
							<label class="text-att">TCC</label><input type="text" name="tcc"  value="<?php echo $donn1['TTC']; ?>" class="des"  required/>
							<label class="text-att">HT DMM </label><input type="text" name="dmm"  value="<?php echo $donn1['HT_DDM']; ?>" class="des"  required />
							<label class="text-att">Timbre</label><input type="text"  value="<?php echo $donn1['TIMBRE']; ?>" name="timbre" class="des"  required/>
							<input type="hidden"  name="ident"  value="<?php echo $_POST['id']; ?>" />
						</div>
							<div id="color_qt">
                        ...
                        <input type="button" value="supprimer" id="sup" class="color" >
						 ...
						 <div id="btn-modele">
                        <input type="submit" value="envoyer" name="valider" id="btn-save-com">
