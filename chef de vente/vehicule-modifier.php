 ...
 <p id="title" >Nissan Djurdjura motors</p>	
	...
	<div id="section">
		...
		<h2 id="titre_de_recherche">recherche</h2><img id="loupe" src="images/loupe.png">
			<form  action="vehicule-modifier.php" method="POST">
...
try {$bdd = new pdo('mysql:host=localhost;dbname=nissan', 'root', '');	} catch (Exception $e) {die('Erreur : '.$e->getMessage()); } 
                       ...
                       while(($donnee=$req1->fetch()))
                       {
								 	if (preg_match("#$_POST[modele]#i","$donnee[designation]"))
								 	 {
								 		echo"<form action='vehicule-modifier-post.php' method='POST'>";
      
											echo '<tr class="resultat">
												    <td>'.$donnee['id_modele_veh'].'</td>
												    <input  type="hidden" name="numero" value='.$donnee['id_modele_veh'].' id="nomero1"/>
													...
													<td> <input type="submit" value="M"  name="supprime" id="input_modif" />
													<input type="hidden"  name="id" value="'.$donnee['id_modele_veh'].'"  /></form></td>
											</tr>';
