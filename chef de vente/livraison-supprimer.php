               ...
               )
{
   ...
   $req1=$bdd->query('SELECT  * FROM command, client, bon_livraison WHERE (bon_livraison.id_command=command.id_command) and  (command.id_client=client.id_client)');  
      ...
      echo '<tr class="resultat" ondblclick="takfa1('.$donnee['id_bon_livraison'].','.$donnee['id_command'].')"  onclick="client('.$donnee['id_bon_livraison'].');">
...
try {
   ...
   $req4=$bdd1->query('select  * from command ,client ,item ,bon_livraison where (item.id_command = command.id_command) and (command.id_client=client.id_client) and (bon_livraison.id_command=command.id_command)');
        ...
        echo '<tr class="resultat" ondblclick="takfa1('.$donnee4['id_bon_livraison'].','.$donnee4['id_command'].')" onclick="client('.$donnee4['id_bon_livraison'].');">
...
function client(str) {
    if (str == "") {
     ...
     var xmlhttp = new XMLHttpRequest();
        ...
        xmlhttp.open("POST","livraison_ajax.php");
...
window.location.href=("livraison-suprimer_post.php?id_liv="+ta+"&id_c="+ta1);
}
