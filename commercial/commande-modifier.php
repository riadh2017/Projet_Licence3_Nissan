...
<div id="container">
 ...
 <p id="title" >Nissan Djurdjura motors</p>	
	...
	<div id="left1">
			...
			<form action="commande-modifier.php" method="POST">
               ...
               )
{
   ...
   $req1=$bdd->query('SELECT  command.id_client, command.id_command, client.nom, command.id_user, command.mode_de_vente, command.date_command ,client.tele,command.numero_bon FROM command, client WHERE (command.id_client=client.id_client)');
				...
				echo"<form action='modifier_command_post.php' method='POST'>";
			...
			echo '<tr class="resultat">
					...
					<input type="hidden" name="id_command1"  value="'.$donnee['id_command'].'"/>
...
<input  type="hidden" name="numero" value="'.$donnee3['numero_bon'].'" id="nomero1"/>
					...
					<input type="hidden" name="id_command1"  value="'.$donnee3['id_command'].'"/>
					<input type="hidden" name="id_client"  value="'.$donnee3['id_client'].'"/>
					<td> <input type="submit"value="V" id="input_modif"/></form></td>
			</tr>';		
...
if(($_GET['message']=='1')){
	...
	echo"<script>alert('le bon de commande e ete  modifier avec succe');</script>";
