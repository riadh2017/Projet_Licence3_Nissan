		...
		<h2 id="titre_de_recherche">recherche</h2><img id="loupe" src="images/loupe.png">
			...
			<div id="right">
				...
				<tr id="titre_de_tab">
					...
					<td class="nom1">Nom</td>
					<td class="tel1">Telephone</td>
					<td class="modele1">vehicule</td>
					<td class="vendeur1">Date</td>
					<td class="mode1">Matricul</td>
					<td class="date1">N° chassis</td>
               ...
               )
{
   ...
   $req1=$bdd->query('SELECT  * FROM command, client, bon_livraison WHERE (bon_livraison.id_command=command.id_command) and  (command.id_client=client.id_client)');
                     ...
                     if   ( $donnee1=$req2->fetch()){
                         ...
                         if(((preg_match("#$_POST[date_du_bon]#i","$donnee[date_livraison]"))and($_POST['date_du_bon']!= NULL))
                         or
                     ...
                     $req21=$bdd->query('select * from matricul_chassis where (id_item=\'' . $donnee1['id_item'] . '\')');
                           $chas=$req21->fetch();
                          

			

			echo '<tr class="resultat" onclick="client('.$donnee['id_bon_livraison'].');">

				    <td>'.$donnee['numero_bon'].'<input  type="hidden" name="numero" value="'.$donnee['id_bon_livraison'].'" id="nomero1"/></td>
...
try {
   ...
   $req3=$bdd1->query('select  * from command ,client ,item ,bon_livraison where (item.id_command = command.id_command) and (command.id_client=client.id_client) and (bon_livraison.id_command=command.id_command)');
...
$i=0;
            ...
            $j=1;
                     ...
                     $req21=$bdd1->query('select * from matricul_chassis where (id_item=\'' . $donnee3['id_item'] . '\')');
                           $chas=$req21->fetch();
                        
			echo '<tr class="resultat" onclick="client('.$donnee3['id_bon_livraison'].');">
				    <td>'.$donnee3['numero_bon'].'<input  type="hidden" name="numero" value="'.$donnee3['id_bon_livraison'].'" id="nomero1"/></td>
					<td class="nom"><p>'.$donnee3['nom'].'</td>
					<td  class="tel" "><p>'.$donnee3['tele'].'</p></td>
					<td class="modele"><p> '.$donnee3['designation'].'</p></td>
					<td class="vendeur"><p>'.$donnee3['date_livraison'].'</p></td>
					<td class="mode"><p>'.$chas['matricul'].'</p></td>
					<td class="date"><p> '.$chas['chassis'].'</td>
...
function client(str) {
    if (str == "") {
     ...
     var xmlhttp = new XMLHttpRequest();
        ...
        xmlhttp.open("POST","livraison_ajax1.php");
