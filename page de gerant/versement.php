	...
	<div id="left">
		<h2 id="titre_de_recherche">recherche</h2><img id="loupe" src="images/loupe.png">
			...
			<div id="right">
				...
				<tr id="titre_de_tab">
                    <td class="numero">N° bon</td>
					<td class="nom">nom</td>
					<td class="tel">telephone</td>
					<td class="modele">vehicule</td>
					<td class="vendeur">date</td>
					<td class="date">Somme</td>
					<td  >versement</td>
					<td class="payer">Reste</td>
               ...
               )
{
   ...
   $req1=$bdd->query('SELECT * FROM command, client, recu_verssement WHERE (recu_verssement.id_command=command.id_command) and  (command.id_client=client.id_client)');
                     ...
                     if   ( $donnee1=$req2->fetch()){
                         ...
                         if(((preg_match("#$_POST[date_du_bon]#i","$donnee[date_de_verssement]"))and($_POST['date_du_bon']!= NULL))
                         or
			...
			echo '<tr class="resultat" onclick="client('.$donnee['id_rv'].');">
				    <td>'.$donnee['numero_bon'].'<input  type="hidden" name="numero" value="'.$donnee['id_rv'].'" id="nomero1"/></td>
					<td class="nom"><p>'.$donnee['nom'].'</td>
					<td  class="tel" "><p>'.$donnee['tele'].'</p></td>
					<td class="modele"><p> '.$donnee1['designation'].'</p></td>
					<td class="vendeur"><p>'.$donnee['date_de_verssement'].'</p></td>
					<td class="mode"><p>'.$donnee['montat'].'</p></td>
					<td class="date"><p> '.$donnee['versement'].'</td>
					<td class="date"><p> '.$donnee['rest_payer'].'</td>
...
try {
   ...
   $req3=$bdd1->query('select  * from command ,client ,item ,recu_verssement where (item.id_command = command.id_command) and (command.id_client=client.id_client) and (recu_verssement.id_command=command.id_command)');
			...
			echo '<tr class="resultat" onclick="client('.$donnee3['id_rv'].');">
				    <td>'.$donnee3['numero_bon'].'<input  type="hidden" name="numero" value="'.$donnee3['id_rv'].'" id="nomero1"/></td>
					<td class="nom"><p>'.$donnee3['nom'].'</td>
					<td  class="tel" "><p>'.$donnee3['tele'].'</p></td>
					<td class="modele"><p> '.$donnee3['designation'].'</p></td>
					<td class="vendeur"><p>'.$donnee3['date_de_verssement'].'</p></td>
					<td class="mode"><p>'.$donnee3['montat'].'</p></td>
					<td class="date"><p> '.$donnee3['versement'].'</td>
					<td class="date"><p> '.$donnee3['rest_payer'].'</td>
...
function client(str) {
    if (str == "") {
     ...
     var xmlhttp = new XMLHttpRequest();
        ...
        xmlhttp.open("POST","versement_ajax1.php");
